...
plot_prior_density <- function(
  ...
  hdci_width = c(0.5, 0.85, 0.99),   # HDCI widths to plot
  ...
  #         HELPER FUNCTIONS
  # ==================================
  .plot_samples <- function(prior_samples, hdci_width, lowerbound, cutoff_hdci, max_kde_n, min_kde_n, hdci_table, subtitle = TRUE) {
    ...
    p <- p + 
      ggplot2::labs(
        subtitle = if(subtitle) stringr::str_wrap(sub, subtitle_width) else waiver(),
  ...
  plots$prior <- .plot_samples(prior_samples, hdci_width, lowerbound, cutoff_hdci, max_kde_n, min_kde_n, hdci_table) + 
    labs(
  ...
  if (param == "sigma") {
    if (any(prior_samples < 0)) stop("negative prior samples produced for random effect sd, try setting truncate_lower = 0")
    prior_samples <- rnorm(nsamples, mean = 0, sd = prior_samples)
    plots$random_intercept <- .plot_samples(prior_samples, hdci_width, lowerbound, cutoff_hdci, max_kde_n, min_kde_n, hdci_table, subtitle = FALSE) + 
      labs(
  ...
  if (link != "identity") {
    ...
    plots$backtransformed <- .plot_samples(prior_samples, hdci_width, lowerbound, cutoff_hdci, max_kde_n, min_kde_n, hdci_table, subtitle = FALSE) + 
...
plot_prior_density_dots <- function(
  ...
  require(stringr)
  require(patchwork)
  .plot_samples <- function(samples, ndots, point_interval, interval_widths, interval_colors, na_color, subtitle = TRUE, vline_color = NULL) {
  ...
  if (param == "sigma") {
    if (any(prior_samples < 0)) stop("negative prior samples produced for random effect sd, try setting truncate_lower = 0")
    prior_samples <- rnorm(nsamples, mean = 0, sd = prior_samples)
    plots$random_intercept <- .plot_samples(prior_samples, ndots, point_interval, interval_widths, interval_colors, na_color, subtitle = FALSE) + 
      labs(
        x = latex2exp::TeX("\\Delta"), 
        title =  latex2exp::TeX("\\Delta ~ Normal(0, \\sigma)")
      )
  ...
  if (link != "identity") {
    ...
    plots$backtransformed <- .plot_samples(prior_samples, ndots, point_interval, interval_widths, interval_colors, na_color, subtitle = FALSE, vline_color = vline_col) + 
    labs(x = latex2exp::TeX(xlab)) 
    message(paste0("inverse link function applied [link: ", link, "; inverse link: ", inv_link), "]")
  }
  # =====> RETURN PLOTS
